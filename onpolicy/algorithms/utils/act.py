...
class ACTLayer(nn.Module):
    ...
    def distill_evaluate_actions(self, x, action, available_actions=None, active_masks=None):
        assert self.grid_goal and self.use_grid_simple and not self.grid_goal_simpler
        x = self.to_input(x)
        regions = action[:, 0].unsqueeze(dim = 1).long()
        points = action[:, 1:]
        ...
        region_probs = region_logits.probs
        point_mean = point_logits.mean
        point_std = point_logits.stddev
        
        dist_rep = torch.cat([region_probs, point_mean, point_std], dim=-1)

        return action_log_probs, dist_entropy, dist_rep